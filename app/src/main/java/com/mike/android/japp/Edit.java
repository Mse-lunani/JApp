...
public class Edit extends AppCompatActivity implements
    ...
    /** EditText field to enter the entry's title */
    private EditText mTitle;

    /** EditText field to enter the journal entry */
...
// If the intent DOES NOT contain a Entry content URI, then we know that we are
        ...
        mEntry = (EditText) findViewById(R.id.editText2);
    }
    ...
    private void saveEntry() {
        ...
        String EntryString = mEntry.getText().toString().trim();


        // Check if this is supposed to be a new Entry
        // and check if all the fields in the editor are blank
        if (mCurrentEntryUri == null &&
                TextUtils.isEmpty(TitleString) && TextUtils.isEmpty(EntryString)) {
        ...
        values.put(Entry.COLUMN_ENTRY_TITLE, TitleString);
        values.put(Entry.COLUMN_ENTRY, EntryString);
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        ...
        if (cursor.moveToFirst()) {
            ...
            int nameColumnIndex = cursor.getColumnIndex(Entry.COLUMN_ENTRY);
            int TitleColumnIndex = cursor.getColumnIndex(Entry.COLUMN_ENTRY_TITLE);

                       // Extract out the value from the Cursor for the given column index
            String Tname = cursor.getString(TitleColumnIndex);
            String  Tentry= cursor.getString(nameColumnIndex);

            // Update the views on the screen with the values from the database
            mTitle.setText(Tname);
            mEntry.setText(Tentry);
